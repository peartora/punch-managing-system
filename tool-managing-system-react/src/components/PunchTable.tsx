...
import useDisplay from "@/common/hooks/use-display";

function PunchTable() {
  const { data, isLoading, isError, refetch } = useDisplay();
    ...
    return (
      <tbody>
        <tr>
          <th>
            에러{" "}
            <button type="button" onClick={() => refetch(undefined)}>
              다시시도
            </button>
          </th>
        </tr>
  ...
  // const handlerChangeForAllCheckBox = (event: any) => {
  //   setChecked((checked) => {
  ...
  // const handlerChangeForSingleCheckBox = (event: any, punchId: string) => {
  ...
  // const handlerSubmitForUsageNumber = () => {
  //   const targetRows: Record<string, unknown>[] = rows
  //     .filter((row) => row.isSelected === true)
  //     .map((row) => {
